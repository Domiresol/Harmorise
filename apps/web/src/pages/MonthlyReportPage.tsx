...
const MOCK_MONTHLY: Record<string, {
  period: string; totalMin: number; prevDiff: number | null;
  practicedays: number; bestStreak: number;
  instruments: { name: string; min: number; color: string }[];
  topBpmSong: { title: string; from: number; to: number };
  dayHeatmap: number[];   // 요일(0=월~6=일) 별 평균 분
  hourHeatmap: number[];  // 시간대(0=0시~23시) 별 빈도
}> = {
  m1: {
    period: '2026년 5월', totalMin: 760, prevDiff: +120,
    practicedays: 18, bestStreak: 12,
    instruments: [
      { name: '기타',   min: 620, color: '#0EA5E9' },
      { name: '피아노', min: 100, color: '#2DD4BF' },
      { name: '기타(직접)', min: 40, color: '#7DD3FC' },
    ],
    topBpmSong: { title: 'Blackbird', from: 68, to: 84 },
    dayHeatmap:  [42, 55, 48, 30, 60, 70, 10],
    hourHeatmap: [0,0,0,0,0,0,0,5,10,8,5,3, 5,2,1,0,0,15,20,18,12,8,5,2],
  },
  m2: {
    period: '2026년 4월', totalMin: 640, prevDiff: -80,
    practicedays: 15, bestStreak: 8,
    instruments: [
      { name: '기타',   min: 530, color: '#0EA5E9' },
      { name: '피아노', min: 110, color: '#2DD4BF' },
    ],
    topBpmSong: { title: '사계 - 봄', from: 90, to: 108 },
    dayHeatmap:  [35, 50, 40, 25, 55, 60, 5],
    hourHeatmap: [0,0,0,0,0,0,0,3,8,5,2,1, 4,2,0,0,0,12,18,15,10,6,3,1],
  },
};

/* ── 도넛 차트 (SVG) ────────────────────────────────────── */
function DonutChart({ data }: { data: { name: string; min: number; color: string }[] }) {
...
export function MonthlyReportPage() {
  ...
  const report = id ? MOCK_MONTHLY[id] : null;
  ...
  return (
    <PageLayout title="월간 리포트" showBack>
      <div className="flex flex-col gap-4 pb-6">
        ...
        {/* ── 헤더 요약 ─────────────────────────────────────── */}
        <Card highlighted>
          <p className="text-lg font-bold text-slate-900 mb-3">{report.period}</p>
        ...
        <Card>
          ...
          <div className="flex items-center justify-between py-1">
            ...
            <div className="flex items-center gap-2 text-sm">
              <span className="text-slate-400">{report.topBpmSong.from} BPM</span>
              <span className="text-slate-300">→</span>
              <span className="font-bold text-primary">{report.topBpmSong.to} BPM</span>
              <span className="bg-primary-pale text-primary text-xs font-bold px-2 py-0.5 rounded-pill">
                +{report.topBpmSong.to - report.topBpmSong.from}
              </span>
            </div>
          </div>
        </Card>
