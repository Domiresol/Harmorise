...
// ── 디자인 토큰 (디자인팀) ────────────────────────────────────
const LEVEL_CONFIG: Record<DayLevel, { bg: string; label: string; emoji: string }> = {
  ...
  return (
    <PageLayout title="연습 캘린더" hasTabBar>
      ...
      <Card highlighted className="mb-4 flex items-center justify-between">
        <div>
          <p className="text-xs text-slate-500 mb-1">현재 스트리크</p>
          <StreakBadge count={streak?.currentStreak ?? 0} />
        </div>
        <div className="text-right">
          <p className="text-xs text-slate-400">최장 기록</p>
          <p className="text-sm font-bold text-slate-700">🏆 {streak?.longestStreak ?? 0}일</p>
        </div>
      </Card>
      ...
      <Card className="mb-4">
        ...
        <div className="flex items-center justify-between mb-4">
          <button onClick={prevMonth} className="p-1 text-slate-500 active:opacity-60">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="15 18 9 12 15 6"/></svg>
          </button>
          <p className="text-md font-bold text-slate-900">{year}년 {month + 1}월</p>
          <button onClick={nextMonth} className="p-1 text-slate-500 active:opacity-60">
        ...
        <div className="grid grid-cols-7 mb-1">
          {DAYS_KR.map((d, i) => (
            <p key={d} className={`text-center text-xs font-medium ${i === 0 ? 'text-red-400' : i === 6 ? 'text-blue-400' : 'text-slate-400'}`}>{d}</p>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-7 gap-y-1">
            {cells.map((day, i) => {
              ...
              const isToday = key === formatKey(today.getFullYear(), today.getMonth(), today.getDate());
        ...
        <div className="mb-4">
          <div className="flex items-center gap-2 mb-2">
            <p className="text-sm font-semibold text-slate-700">
              {parseInt(selected.split('-')[1])}월 {parseInt(selected.split('-')[2])}일 기록
            </p>
            {selectedDay && (
              <span className={`text-xs px-2 py-0.5 rounded-full text-white font-medium ${LEVEL_CONFIG[selectedDay.level].bg}`}>
                {LEVEL_CONFIG[selectedDay.level].emoji} {LEVEL_CONFIG[selectedDay.level].label}
              </span>
            )}
          </div>

          {selectedSessions.length === 0 ? (
            <p className="text-sm text-slate-400 text-center py-4">이 날은 연습 기록이 없어요</p>
          ) : (
                ...
                <Card key={s.id} padding="sm">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      {s.bpm && (
                        <p className="text-xs text-slate-400 mt-0.5">🥁 {s.bpm} BPM</p>
                      )}
                    </div>
                    <span className="text-sm font-bold text-primary">{formatMinutes(s.durationMinutes)}</span>
                  </div>
                </Card>
              ))}
              {selectedDay && (
      ...
      {/* ── 이달 요약 ─────────────────────────────────────── */}
      <Card className="mb-4">
        <p className="text-sm font-bold text-slate-800 mb-3">{month + 1}월 요약</p>
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-primary-pale rounded-item p-3 text-center">
                <p className="text-2xl font-bold text-primary">{summary?.practicedDays ?? 0}</p>
                <p className="text-xs text-slate-500 mt-0.5">연습 일수</p>
              </div>
              <div className="bg-accent-light rounded-item p-3 text-center">
                <p className="text-2xl font-bold text-teal-600">{formatMinutes(summary?.totalMinutes ?? 0)}</p>
            ...
            <div className="flex gap-2 justify-center">
              {(['perfect', 'great', 'good'] as DayLevel[]).map(l => (
                <div key={l} className="flex-1 rounded-item p-2 text-center bg-slate-50">
                  <p className="text-lg font-bold text-slate-800">
                    {l === 'perfect' ? (summary?.perfectDays ?? 0) :
                     l === 'great'   ? (summary?.greatDays   ?? 0) :
                                       (summary?.goodDays    ?? 0)}일
                  </p>
